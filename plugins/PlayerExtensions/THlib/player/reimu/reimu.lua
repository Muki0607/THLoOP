...
function reimu_player:shoot()
    ...
    New(reimu_bullet_red, 'reimu_bullet_red', self.x + 10, self.y, 24, 90, 2 * dmgfix3)
    New(reimu_bullet_red, 'reimu_bullet_red', self.x - 10, self.y, 24, 90, 2 * dmgfix3)
...
function reimu_player:spell()
    ...
    if self.slow == 1 then
        ...
        New(reimu_kekkai, self.x, self.y, 1.25 * 1.5, 12, 20, 12) --灵梦的b伤害太低了得加强一下
        self.nextspell = 240
        self.protect = p
    else
        PlaySound('nep00', 0.8)
...
function reimu_sp_ef1:kill()
    ...
    self.vscale = 2
    self.hscale = 2
    --灵梦的b不知道为什么打不到人，只能模仿原作在炸的时候给一个全屏伤害了
    for _, g in ipairs({ GROUP_ENEMY, GROUP_NONTJT }) do
        for _, o in ObjList(g) do
            if o.colli then
                if o.class.base.take_damage then o.class.base.take_damage(o, 25) end
            end
        end
    end
