...
function marisa_player:init(slot)
    ...
    self.Missile = function(idx, dmgfix3)
        PlaySound('msl', 0.2)
        for _, i in ipairs(idx) do
            if self.sp[i] and self.sp[i][3] > 0.5 then
                New(marisa_missile, 'marisa_missile',
                    ...
                    16, 90, self.dmglist[i] * 7 * dmgfix3)
            end
        end
    end
...
function marisa_player:shoot()
    ...
    if self.nextspell <= 0 then
        if self.timer % nextshoot == 0 then
            ...
            New(marisa_bullet, 'marisa_bullet', self.x + 6, self.y, 24, 90, 2 * dmgfix3)
            New(marisa_bullet, 'marisa_bullet', self.x - 6, self.y, 24, 90, 2 * dmgfix3)
        end
        local power = int(lstg.var.power / 100)
        if self.slow == 1 then
            if power <= 2 then
                if self.timer % 16 == 0 then
                    self.Missile({ 1, 2, 3, 4 }, dmgfix3)
                end
            elseif power == 3 then
                if self.timer % 16 == 0 then
                    self.Missile({ 1, 3 }, dmgfix3)
                end
                if self.timer % 16 == 8 then
                    self.Missile({ 2 }, dmgfix3)
                end
            else
                if self.timer % 16 == 0 then
                    self.Missile({ 1, 4 }, dmgfix3)
                end
                if self.timer % 16 == 8 then
                    self.Missile({ 2, 3 }, dmgfix3)
                end
            end
        else
