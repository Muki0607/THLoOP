...
laser_bent_renderFunc = {
    ...
    ['th19'] = function(self)
        ...
        if self.timer < self._l * 4 and self.node then
            local c = Color(self._a, self._r, self._g, self._b)
            SetImageState('laser_node8', self._blend, c)
            Render('laser_node8', self.prex, self.prey, -3 * self.timer, (8 + self.timer % 3) * 0.125 * self.node / 8)
            Render('laser_node8', self.prex, self.prey, -3 * self.timer + 180, (8 + self.timer % 3) * 0.125 * self.node / 8)
        end
    end,
}
