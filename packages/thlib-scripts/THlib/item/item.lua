...
LoadTexture('AiC_square', 'THlib/item/item.png')
LoadImage('AiC_square', 'AiC_square', 0, 0, 67, 67)
...
function item_power:frame()
    ...
    if t and IsValid(t) then
        ...
        if t ~= player and Dist(self, t) <= hypot(a, b) then
            Kill(self)
            PlaySound('item00', 0.3, self.x / 200)
        end
    end
...
function item_power:render()
    ...
    elseif self.is_power_blue then
        SetImageState(self.img, '', Color(255, 170, 160, 247))
    else
...
function item_power_large:init(x, y, v, a)
    ...
    --[[self.img = 'AiC_square_overcharged'
    self.hscale = 0.18
    self.vscale = 0.18]]
end
...
--注意：power_full改为过充魔力道具
item_power_full = Class(item)
function item_power_full:init(x, y, v, a)
...
function item_power_full:frame()
    local player = self.target
    if self.timer < 24 then
        self.rot = self.rot + 45
        self.hscale = (self.timer + 25) / 48
    ...
    elseif self.attract > 0 then
        ...
        --self.vy = max(self.dy - 0.03 * 0.75, -1.7 * 0.75)
    end
    if self.y < lstg.world.boundb then
        Del(self)
    end
...
function item_chip:collect()
    lstg.var.hp = lstg.var.hp + 20
    PlaySound('aic_heal', 0.5)
...
function item:EndChipBonus(x, y)
    if self.chip_bonus and self.bombchip_bonus then
        ...
        --New(item_chip, x, y)
        --_drop_item(item_power_large, 25, x, y)
    else
...
function item.PlayerInit()
    lstg.var.power = 100
    lstg.var.maxpower = 500
    lstg.var.maxpower2 = 500
    --lstg.var.lifeleft = 2
    lstg.var.hp = 100
    lstg.var.maxhp = 400
    lstg.var.temp_hp = 0
...
function item:PlayerMiss()
    ...
    if lstg.var.hp > 0 then
        --[[for i = 1, 7 do
            local a = 90 + (i - 4) * 18 + self.x * 0.26
            New(item_power, self.x, self.y + 10, 3, a)
        end]]
        if CheckEnhancer(8) then
            lstg.var.power = 0
            item.DropItem(self.x, self.y, { 99, 0, 0 })
            item.DropItem(self.x, self.y, { 1, 0, 0 })
        elseif not CheckEnhancer(2) then
            if lstg.var.power <= 320 then
                item.DropItem(self.x, self.y, { 64, 0, 0 })
            elseif lstg.var.power < 500 then
                item.DropItem(self.x, self.y, { int(lstg.var.power / 5), 0, 0 })
            else
                --防止一次性掉100个直接合成大p点，使4.00p原地蒸发
                item.DropItem(self.x, self.y, { 99, 0, 0 })
                item.DropItem(self.x, self.y, { 1, 0, 0 })
            end
            lstg.var.power = 0
        ...
        if CheckDiff(4) or CheckEnhancer(16) then
            p = 1
        elseif CheckDiff(3) then
            p = 0.9
        elseif CheckDiff(2) then
            p = 0.75
        else
            p = 0.5
        end
        ...
        if p >= ran:Float(0, 1) then
            if lstg.var.maxpower - b >= 200 then
                lstg.var.maxpower = max(200, lstg.var.maxpower - b)
            else
                local d = b + 200 - lstg.var.maxpower
                lstg.var.maxpower = 200
                lstg.var.maxpower2 = max(0, lstg.var.maxpower - d)
            end
        end
        lstg.var.exmp = max(lstg.var.exmp, 100)
    else
...
function item.PlayerGraze()
    ...
    lstg.var.dodge = min(lstg.var.dodge + 1, 600)
end
