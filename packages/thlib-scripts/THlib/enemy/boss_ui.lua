...
function boss.ui:init(system, b)
    ...
    self.hpbar = aic.ui.hpbar(self, self.system)
    ...
    if _ui.drawname then
        ...
        if row >= 0 then
            for i = 0, row do
                --确定排数
                local col
                if i == row then col = sc_left % 8
                else col = 8 end
                if col == 0 then col = 8 end
                --渲染cardleft
                for j = 1, col do
                    local scleft, s = "boss_cardleft", 0.5
                    if _sc_left_type[sc_left - i * 8 - j + 1] == 'time' then
                        scleft = "boss_cardleft_time"
                        s = 0.4
                        y = y - 2
                    elseif _sc_left_type[sc_left - i * 8 - j + 1] == 'great' then
                        scleft = "boss_cardleft_great"
                    end
                    Render(scleft, x + j * 12, y - i * 12, 0, s)
                end
            end
            --[[y = y - m * 12
            for i = 1, m2 do
                local scleft = "boss_cardleft"
                if _sc_left_type[sc_left - 8 * m - i] == 'time' then
                    scleft = "boss_cardleft_time"
                elseif _sc_left_type[sc_left - 8 * m - i] == 'great' then
                    scleft = "boss_cardleft_great"
                end
                SetImageState(scleft, "", Color(0xFFFFFFFF))
                Render(scleft, x + i * 12, y, 0, 0.5)
            end
            local t, at, x2, y2
            ...
            if self.t > 0 then
                ...
                y2 = y - t / 5
                local scleft = "boss_cardleft"
                if _sc_left_type[sc_left] == 'time' then
                    scleft = "boss_cardleft_time"
                elseif _sc_left_type[sc_left] == 'great' then
                    scleft = "boss_cardleft_great"
                end
                SetImageState(scleft, "",
                        Color(255 * (1 - (t / at)), 255, 255, 255))
                Render(scleft, x2, y2, 0, 0.5 + (t / at) * 0.5)
            end]]
        end
    end
end
